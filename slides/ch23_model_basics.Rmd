...
|formula|model|説明|
|---|---|---|
|`y~x+I(x^2)`|$y\sim a_0+a_1x+a_2x^2$|線形でない、二乗項を使った回帰|
|`y~poly(x, 3)`|$y\sim a_0+a_1+a_2x^2+a_3x^3$|三次回帰|
|`I(log(y))~I(log(x))`|$\log y\sim a_0 + a_1 \log x$|もうめちゃくちゃ重要|

## いろいろと理論的なよもやま話

- 多項式モデル `poly(x, p)` を使って、多項式の次数 $p$ を無限大にすると任意の曲線を近似できちゃう（テイラーの定理）
- 多項式の次元を大きくすると $x$ が極端に大きいときは急速に大きく/小さく なるので現実と合わない
